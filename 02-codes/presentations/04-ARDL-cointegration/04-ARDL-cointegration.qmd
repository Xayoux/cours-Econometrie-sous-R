...
```{r}
# Transfiomer les séries temporelles en un objet tibble
df <- 
  df_ts |> 
  timetk::tk_tbl(
    rename_index = "date" # La variable contenant les indices temporels est renommée "date"
  ) |> 
  print()
```
...
```{r}
# Représenter graphiquement nos variables
df |> 
  ...
  pivot_longer(
    cols = !c(date), # Pivoter toutes le svariables sauf la date
    names_to = "variable",
    values_to = "value"
  ) |> 
  print() |> 
...
if (rlang::is_installed("dobby")) {
  ...
  purrr::map2_dfr(
    list(df$LRM, df$LRY, df$IBO, df$IDE), # Séries à tester
    c("LRM", "LRY", "IBO", "IDE"), # Noms des séries
    # Créer une fonction prenant deux paramètres : série et name
    # Cette fonction exécute la fonction adf_test_auto()
    \(serie, name) dobby::adf_test_auto(
      serie, 
      name, 
      lags = 20, 
      message = FALSE, # Ne pas afficher les messages de résultats
      return_res = TRUE # Retourner un tableau de résultat
    )
  )
}
```

Les tests ADF nous indiquent que les séries LRY et IBO ne semblent pas stationnaires. Leur inclusion dans un modèle utilisant les MCO peut résulter en une corrélation fallacieuse. Les coefficients associés ont de très fortes chances d'être biaisés fortement. Si le package `dobby` n'est pas installé, le test ADF peut se faire manuellement avec la fonction `urca::ur.df()`. Attention, les p.values indiquées ne sont pas les bonnes, et les valeurs critiques données par la fonction concernent le test joint ADF et pas le test individuel (que l'on préfère généralement). Pour plus d'informations sur le test ADF et une démonstration de `urca`, voir *03-ARIMA*.

## Régression statique

Dans un premier temps, modélisons la relation (possiblement fallacieuse) suivante :
...
dynlm::dynlm(
  ...
  data = df_ts # Obligé d'utiliser un objet Time Series pour faire la différenciation
) |> 
  summary()
```
...
if (rlang::is_installed("dobby")) {
  dobby::check_univariate_autocorr(
    reg_statique$residuals, # Série des résidus à tester
    "résidus statiques", # Nom de la série
    test_type = "Ljung-Box", # Type de test
    return_output = FALSE # Ne pas retourner de tableau de résultats
  )
}
```
...
if (rlang::is_installed("dobby")) {
  dobby::check_univariate_autocorr(
    reg_dep_retard$residuals, 
    "résidus dépendante retardée", 
    test_type = "Ljung-Box", 
    return_output = FALSE
  )
} 
...
models <- 
  ARDL::auto_ardl(
    LRM ~ LRY + IBO + IDE, # Variables en niveau à utiliser
...
```{r}
# Breusch-Pagan test
lmtest::bptest(ardl_3132) # La fonction s'applique sur le modèle directement
...
```{r}
fDMA::archtest(
  as.numeric(ardl_3132$residuals**2), # La fonction s'applique sur la série des résidus mise au carré
  lag = 20
)
```
...
)[["tab"]] # Sélectionner le tableau de résultat
```
...
ARDL::multipliers(
  ...
  ARDL::plot_delay(interval = 0.95) # Représenter graphiquement les impacts temporaires directs avec un intervalle de confiance de 95%
```
...
ce <- 
  ARDL::coint_eq(
    ardl_3132, # Modèle utiliser pour calculer l'équation de cointégration. La première variable du jeu de données utilisée doit être la variable dépendante (c'est nul)
...
ARDL::plot_lr(
  ardl_3132, # Modèle utilisé
  coint_eq = ce, # Equation de long-terme calculée
  show.legend = TRUE # Afficher une légence
)
```

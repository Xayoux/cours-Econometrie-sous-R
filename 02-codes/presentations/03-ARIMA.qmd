...
dobby::adf_test_auto(sp_500, "S&P500", lags = 20, return_res = FALSE)
```
...
```{r}
# Test ADF automatique
dobby::adf_test_auto(rend_sp_500, "S&P500", lags = 20, return_res = FALSE)
```
...
On regarde ensuite l'auto-corrélation de la série avec les tests de Ljung-Box et Box-Pierce avec des ordres d'auto-corrélation de 1 et 20. Le package maisons `dobby` utilise la fonction `stats::Box.test()` mais permet d'avoir des résultats pouvant être extraits entre autre.

```{r}
# Test de Ljung-Box
dobby::check_univariate_autocorr(rend_sp_500, "S&P500", test_type = "Ljung-Box", return_output = FALSE)

# Test de Box Pierce
dobby::check_univariate_autocorr(rend_sp_500, "S&P500", test_type = "Box-Pierce", return_output = FALSE)
```

Les tests de Box-Pierce et Ljung-Box indique tous les deux que la série présente de l'auto-corrélation à l'ordre 1 et 20 pour de très faibles niveaux de risque. L'objectif va être d'essayer de modéliser au maximum cette auto-corrélation.
...
```{r}
ar_1 <- forecast::Arima(rend_sp_500, order = c(1, 0, 0), include.mean = TRUE)
...
```{r}
dobby::check_univariate_autocorr(ar_1$residuals, "résidus AR(1)")[1]
```

Le test d'auto-corrélation nous indique que toute l'auto-corrélation de long-terme n'a pas été prise en compte. Le modèle n'est donc pas valide.

```{r}
dobby::check_univariate_normality(as.numeric(rend_sp_500), "résidus AR(1)", return_output = FALSE)
```
...
```{r}
# test de white
whitestrap::white_test(ar_1)

# ARCH test
fDMA::archtest(as.numeric(ar_1$residuals**2), lag = 20)
```

L'hypothèse d'homogénéité de la variance est rejetée. La variance des résidus ne semble pas constante dans le temps et semble être conditionnellement auto-corrélée.
...
On peut utiliser la fonction `foreast::auto.arima()` pour déterminer le modèle qui correspond le mieux à nos données selon les critères d'informations
...
On remarque que tous nos coefficients passent le test de surdimmensionnement puisque toutes les t-stats sont supérieures à 1.96. le modèle est donc validé de ce point de vue.

```{r}
dobby::check_univariate_autocorr(best_arma$residuals, "residus")[1]
```

La majorité de l'auto-corrélation de la série semble avoir été prise en compte. Il n'y a pas à court-terme d'auto-corrélation. Par contre, on peut voir que le résultat est moins net à plus long-terme. Ici à l'ordre 20, on rejette l'hypothèse nulle de non-significativité à 5%.
...
Encore une fois, la modélisation ARMA a du mal à reproduire l'amplitude de variation des rendements.

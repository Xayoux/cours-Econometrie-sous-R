...
p_indiv <- 
  ...
  labs(
    ...
    x = "Pays",
    y = "Log(Consommation d'essence moyenne par voiture)"
  )
...
p_temp <- 
  ...
  labs(
    ...
    x = "Année",
    y = "Log(Consommation d'essence moyenne par voiture) [lgaspcar]"
  )
...
poobly::hsiao(
  formula = lrpmg ~ lgaspcar + lcarpcap + lincomep,
  data = df_panel
)
```
...
La premièr étape consiste à tester s'il existe de l'hétérogénéité dans le modèle. On rejette l'hypothèse nulle à un seuil de 5% (p.value < 0.001). Il semble donc qu'il existe de l'hétérogénéité au sein de notre modèle. Il nous faut maintenant en découvrir la forme. 
...
Il faut noter que cette fonction ne teste que pour la présence d'effets individuels et pas d'effets temporels. 

...
Généralement ce que l'on fait pour éviter d'avoir à changer tous les chemins d'accès d'un fichier, on définit un répertoire de travail à partir duquel on va pouvoir écrire les chemins d'accès en relatif.
...
setwd("D:/cours/Econométrie sous R")  
...
Une fois un "Projet R" créé, une manière très simple de créer ses chemins d'accès est d'utiliser la fonction `here` du package `here`. Cette fonction utilisée sans arguments supplémentaires va renvoyer une chaîne de caractère indiquant le chemin d'accès absolu jusqu'au répertoire de travail :
...
Afin de gagner du temps, de la productivité et de permettre une relecture facilité du projet, il est essentiel d'avoir une organisation rigoureuse et propre de son dossier de travail. Chacun dispose de sa manière de travailler et d'organiser les choses et la manière présentée ici n'est peut-être pas celle qui est optimale et va correspondre à tout le monde.
...
-   `README` : Un fichier (Markdown, Word, txt...) qui explique comment exécuter l'analyse et obtenir les résultats présentés dans l'article. Il est important de bien indiqué les packages utilisés, les configurations nécessaires et tout élément nécessaire à la reproduction de l'analyse.
...
-   `02-codes`
    ...
    -   `functions_R` : Dossier dans lequel chaque fonction R créée et utilisée va être enregistrée dans un fichier `.R` spécifique.
    ...
    -   `utils` : Dossier dans lequel vont se trouver tous les scripts utilitaires qui vont être appelés au début de chaque script principal.
        ...
        -   `paths.R` : Code R qui contient des chemins d'accès enregistrés dans des variables afin de ne pas avoir à toujours les renseigner.

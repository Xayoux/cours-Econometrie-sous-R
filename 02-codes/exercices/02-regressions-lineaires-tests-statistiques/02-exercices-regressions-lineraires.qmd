...
quarto::quarto_render(
    here::here(
        ...
        "02-exercices-regressions-lineraires.qmd"
    ), 
    output_format = "all"
)
```
...
## Exercice 2 : Estimation de la valeur d'une maison
...
## Exercice 4 : Education et nombre d'enfants

*L'éducation de la mère affecte-t-elle le nombre d'enfant quelle aura ?*
...
```{r}
ggplot(df, aes(x = educ, y = kids)) +
  geom_point(alpha = 0.4) +
  geom_smooth(method = "lm", color = "red", formula = y~x) 
```

```{r}
model_1 <- lm(kids ~ educ + meduc + feduc + age + black, data = df)
summary(model_1)
```
...
```{r}
model_2 <- lm(kids ~ educ + age + black, data = df)
coeftest(model_2, vcov = hccm(model_2, type = "hc0"))
```
...
```{r}
model_3 <- lm(kids ~ educ +I(educ^2) + age + black, data = df)
coeftest(model_3, vcov = hccm(model_3, type = "hc0"))
```
...
```{r}
effects::effect("educ", model_3, vcov. = hccm) |>
  plot()
```
...
```{r}
coef_educ_model_3 <- model_3$coefficients["educ"]
coef_educ_2_model_3 <- model_3$coefficients["I(educ^2)"]

-coef_educ_model_3 / (2*coef_educ_2_model_3)
```
...
## Exercice 5 : Prix des cigarettes

*Le prix des cigarettes a-t-il un impact sur la consommation journalière ?*
...
```{r}
ggplot(df, aes(x = cigpric, y = cigs)) +
  geom_point(alpha = 0.4) +
  geom_smooth(method = "lm", color = "red", formula = y~x) 
```

```{r}
model_1 <- lm(cigs ~ cigpric, data = df)
summary(model_1)
```
...
```{r}
model_2 <- lm(cigs ~ cigpric + age, data = df)
summary(model_2)
```
...
```{r}
model_3 <- lm(cigs ~ cigpric + age + lincome + white, data = df)
coeftest(model_3, vcov = hccm(model_3, type = "hc0"))
```
...
```{r}
model_4 <- lm(cigs ~ cigpric + lincome + white + white*lincome, data = df)
coeftest(model_4, vcov = hccm(model_4, type = "hc0"))
```
